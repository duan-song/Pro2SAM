    ...
    for epoch in range(0, args.epochs):
        ...
        torch.save({'model': model.state_dict(),
                    ...
                    'epoch': epoch + 1,
                                    ...
                                    args.root + '/' + args.arch + '/' + 'epoch_' + str(epoch + 1) + '.pth.tar'),
        ...
        if args.evaluate:
            ...
            if top1_acc > best_top1:
                ...
                torch.save({'model': model.state_dict(),
                            'best_thr': thr,
                            'epoch': epoch + 1,
                            }, os.path.join(args.save_path, args.root + '/' + args.arch + '/' + 'best_top1.pth.tar'),
                           _use_new_zipfile_serialization=False)
            if gt_acc > best_gt:
                best_gt = gt_acc
                torch.save({'model': model.state_dict(),
                            'best_thr': thr,
                            'epoch': epoch + 1,
                            }, os.path.join(args.save_path, args.root + '/' + args.arch + '/' + 'best_gt.pth.tar'),
                           _use_new_zipfile_serialization=False)
            if loc_acc > best_loc:
                best_loc = loc_acc
                torch.save({'model': model.state_dict(),
                            'best_thr': thr,
                            'epoch': epoch + 1,
                            }, os.path.join(args.save_path, args.root + '/' + args.arch + '/' + 'best_loc.pth.tar'),
                           _use_new_zipfile_serialization=False)
