...
def multiboxes_from_mask(mask, thr=100):
    image = mask
...
if __name__ == '__main__':
    ...
    checkpoint = torch.load('logs/' + args.root+ '_Pro2SAM' + '/' + args.arch + '/' + 'best_gt.pth.tar')
    model.load_state_dict(checkpoint['model'])

    root_path = '/media/gpu/dss/dataset/WSOL/CUB_200_2011/test/'
    save_root = '/media/gpu/dss/code/WSOL/Pro2Mask/Data/CUB_200_2011/CUB_results_Pro2SAM_0.9/'
    ...
    sub_root_list = os.listdir(root_path)
    for class_name in tqdm(sub_root_list, total=len(sub_root_list)):
        class_root = os.path.join(root_path, class_name)
        class_root_list = os.listdir(class_root)

        save_path = os.path.join(save_root, class_name)
        if not os.path.exists(save_path):
            os.makedirs(save_path)

        for image_name in tqdm(class_root_list, total=len(class_root_list)):
            image_path = os.path.join(root_path, class_name, image_name)
            ...
            _, map = model(x=image, phase='test')
            # print(map.shape)

            pred = F.interpolate(map.view(1, 1, 14, 14), size=(224, 224), mode='bilinear', align_corners=True)
            pred = pred[0, 0, :, :].detach().cpu().numpy()
            mask = np.uint8(pred > 0.9) * 255
            cv2.imwrite(save_dir, mask)
            # print('the loc map saving in '+save_dir)
